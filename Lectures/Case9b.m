...
%forward simulate the model: Simple Euler numericla integration. u(1,:) is
%the current in the circuit.
for I = 2:length(t)
    u(3,I) = Vd(I)/L -Cinv/L * u(1,I-1) - R/L * u(2,I-1);
    u(2,I) = u(2,I-1) + u(3,I)*t(2);
    u(1,I) = u(1,I-1) + u(2,I)*t(2);
end
ut=u(1,:);
% add noise to the data:
u(1,:)=u(1,:)+0.1*randn(size(t));
...
A(:,1:3) = [[0 u(1,1:end-2) 0]' [0 0 u(1,1:end-2)]' [Vd(1:end-2) 0 0]'];
spy(A(998:1005,:))
...
uu=AAA*x;
figure(904)
plot(t,ut,[t 10],uu,t(1:3001),u(1,1:3001))
legend('true','simulated','training')
xlabel('\bfTime [s]')
ylabel('\bfCurrent [A]')
figure(905)
